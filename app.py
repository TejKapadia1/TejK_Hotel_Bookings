...
if df is not None:
    ...
    # ============== DATA VISUALIZATION TAB (ULTRA ROBUST) ==============
    with tabs[0]:
        ...
        try:
            if (
                not market_seg_counts.empty
                and 'market_segment' in market_seg_counts.columns
                and (market_seg_counts['market_segment'] > 0).any()
            ):
        ...
        try:
            if not room_counts.empty and (room_counts['assigned_room_type'] > 0).any():
        ...
        try:
            if not special_req.empty and (special_req['total_of_special_requests'] > 0).any():
        ...
        try:
            if not country_counts.empty and (country_counts['country'] > 0).any():
        ...
        try:
            if not booking_chg.empty and (booking_chg['booking_changes'] > 0).any():
        ...
        try:
            if not customer_type_counts.empty and (customer_type_counts['customer_type'] > 0).any():
    ...
    # ============= CLASSIFICATION TAB (unchanged, already robust) =============
    with tabs[1]:
